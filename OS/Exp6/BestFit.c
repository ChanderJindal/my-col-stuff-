...
void MemoryAllocation(struct Stack* MyStack){
for(int i=0;i<MyStack->TotalProcesses;i++){
    int MinMemory = UpperMemoryLimit,MinMemoryIdx = -1;
        for(int j =0;j<MyStack->TotalMemorySlots;j++){
            if(MyStack->MemorySlots[j] >= MyStack->ProcessMemory[i] && MyStack->MemorySlots[j] > -1){
                if(MinMemory > MyStack->MemorySlots[j]){
                    MinMemory = MyStack->MemorySlots[j];
                    MinMemoryIdx = j;
                }
            }
        }
    ...
    MyStack->MemorySlots[MinMemoryIdx] = -(i+1);
