...
class TicTacToe:
    def __init__(self,pick:str,sz:int=3,):
        self.pick = pick
        self.dim_sz = sz
    ...
    def MoveRecord(self,r:int,c:int):
    ...
    def CheckWin(self) -> int: # 1 you won, 0 computer won, -1 tie
        ...
        for i in range(0,self.dim_sz):
            for j in range(0,self.dim_sz):
                ...
                if self.board[i][j] == 'blur':#BLANK
                    ...
                    self.board[i][j] = CompPick#Check if I,Chander (Computer can win)
